...
hPanel → Advanced → Cron Jobs → النوع Custom، **كل دقيقة**:

```
* * * * * /usr/bin/php /home/USER/bahja/artisan schedule:run >> /dev/null 2>&1
```

كل دقيقة لازمة لأن `schedule:run` هو أيضاً من يصرّف طابور البريد (تأكيدات النشرة،
تنبيهات التواصل، اعتماد الفرق) ويُطلق الفعاليات المجدولة. الاستضافة المشتركة
تعطّل `proc_open`، لذلك تُعرَّف المهام في `routes/console.php` بـ `Schedule::call`
وتُشغَّل داخل العملية نفسها؛ لا تستعملوا `Schedule::command` هناك فستفشل بصمت.
للتأكد أن الكرون حيّ: اللوحة → النظام → «صحة النظام» → «نبضة الجدولة».
إن لم يتوفر cron لدى المزود: استخدموا خدمة cron-job.org المجانية لتزور رابطاً سرّياً مشابهاً للخطوة 5 يستدعي `schedule:run`.

## 6.1 البريد (SMTP) بضغطة واحدة

المنصّة ترسل تأكيدات النشرة وتنبيهات «تواصلوا معنا» واعتماد الفرق من صندوق
`info@bahjagaza.com` على هوستينغر. بدل تحرير `.env` يدوياً:

1. في GitHub: Settings → Secrets and variables → Actions → أضيفوا سرّاً باسم
   `MAIL_PASSWORD` بكلمة مرور الصندوق (مرة واحدة).
2. تبويب Actions → «ضبط البريد على الاستضافة» → Run workflow. تضبط المهمة
   `MAIL_*` في `.env` عبر `deploy/set-env.php`، وتعيد بناء الكاش، وترسل رسالة
   اختبار، وتصرّف الطابور، وتضيف كرون `schedule:run` إن كان غائباً.

لتغيير كلمة المرور لاحقاً حدّثوا السرّ وأعيدوا تشغيل المهمة.
